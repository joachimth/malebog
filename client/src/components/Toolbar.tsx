...
const PRESET_COLORS = [
  "#FF0000", "#FF7F00", "#FFFF00", "#00FF00", "#0000FF", "#9400D3", // Rainbow core
  "#FF1493", "#00FFFF", "#8B4513", "#000000", "#FFFFFF" // Essential extras
];
...
export function Toolbar({
  ...
  return (
    <motion.div 
      initial={{ y: 100 }}
      animate={{ y: 0 }}
      className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-white/95 backdrop-blur shadow-2xl rounded-2xl p-2 md:p-3 border border-border/50 flex items-center gap-1.5 md:gap-3"
    >
      {/* Tools Group */}
      ...
      {/* Direct Color Picker - compact row */}
      <div className="flex items-center gap-1 pr-3 border-r border-gray-200">
        {PRESET_COLORS.map(c => (
          <button
            key={c}
            ...
            className={cn(
              "w-6 h-6 md:w-7 md:h-7 rounded-full border-2 transition-all hover:scale-110 active:scale-95",
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button 
              ...
              className="w-6 h-6 md:w-7 md:h-7 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center hover:bg-gray-50"
              title="Vælg selv farve"
            >
              <div className="w-3.5 h-3.5 md:w-4 md:h-4 rounded-full bg-gradient-to-tr from-red-500 via-green-500 to-blue-500" />
            </button>
          </PopoverTrigger>
...
function ToolButton({ active, onClick, icon, color, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, color?: string, label: string }) {
  return (
    <div className="relative group">
      ...
      <span className="text-[9px] font-bold opacity-0 group-hover:opacity-100 transition-opacity absolute -top-6 left-1/2 -translate-x-1/2 bg-black/80 text-white px-1.5 py-0.5 rounded pointer-events-none whitespace-nowrap z-10">
        {label}
      </span>
    </div>
  );
